...
def find_welcome_channel(guild: discord.Guild) -> discord.TextChannel | None:
    """Best-effort auto-detect: look for a text channel whose name contains 'welcome'."""
...
class Utility(commands.Cog):
    ...
    def get_welcome_channel(self, guild: discord.Guild):
        ...
        if channel_id:
            ...
            if channel:
                return channel
        # Fall back to auto-detecting a channel with "welcome" in its name
        # (matches "#welcome😃" style names too, since emoji doesn't break the substring check)
        return find_welcome_channel(guild)
